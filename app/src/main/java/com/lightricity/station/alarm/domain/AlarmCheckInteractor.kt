package com.lightricity.station.alarm.domain
...
import com.lightricity.station.alarm.receiver.CancelAlarmReceiver
import com.lightricity.station.alarm.receiver.MuteAlarmReceiver
import com.lightricity.station.database.tables.Alarm
...
class AlarmCheckInteractor(
    ...
    fun getStatus(sensor: Sensor): AlarmStatus {
        val alarms = getEnabledAlarms(sensor)
        val hasEnabledAlarm = alarms.isNotEmpty()
        alarms
            .forEach {
                val resourceId = getResourceId(it, sensor)
    ...
    private fun getEnabledAlarms(sensor: Sensor): List<Alarm> =
        Alarm
            .getForTag(sensor.id)
            .filter { it.enabled }

    private fun getResourceId(it: Alarm, sensor: Sensor, shouldCompareMovementCounter: Boolean = false): Int {
        return when (it.type) {
            Alarm.TEMPERATURE,
            Alarm.HUMIDITY,
            Alarm.PRESSURE,
            Alarm.RSSI -> compareWithAlarmRange(it, sensor)
            Alarm.MOVEMENT -> {
                val readings: List<TagSensorReading> =
                    TagSensorReading.getLatestForTag(sensor.id, 2)
                if (readings.size == 2) {
                    when {
                        shouldCompareMovementCounter && sensor.dataFormat != null  && readings.first().movementCounter != readings.last().movementCounter -> R.string.alert_notification_movement
    ...
    private fun compareWithAlarmRange(alarm: Alarm, tag: Sensor): Int {
