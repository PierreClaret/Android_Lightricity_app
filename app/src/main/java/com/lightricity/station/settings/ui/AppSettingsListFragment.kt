...
class AppSettingsListFragment : Fragment(R.layout.fragment_app_settings_list), KodeinAware {
    ...
    private fun setupViewModel() {
        viewModel.experimentalFeatures.observe(viewLifecycleOwner, Observer {
            Timber.d("experimentalSettingsLayout $it")
            experimentalSettingsLayout.isVisible = it
        })
    }
    ...
    private fun setupUI() {
        ...
        PDUFormatLayout.setDebouncedOnClickListener {
            (activity as? AppSettingsDelegate)?.openFragment(R.string.settings_pduFormat)
        }
