package com.lightricity.station.app.preferences
...
import com.lightricity.station.util.extensions.isUpdateInstall
import java.util.*
...
class Preferences constructor(val context: Context) {
    ...
    var lightUnit: LightUnit
        get() {
            return when (sharedPreferences.getInt(PREF_LIGHT_UNIT, 0)) {
                0 -> LightUnit.LUMEX
                else -> LightUnit.LUMEX
            }
        }
        set(value) {
            sharedPreferences.edit().putInt(PREF_LIGHT_UNIT, value.code).apply()
        }

    var soundUnit: SoundUnit
    get(){
        return when (sharedPreferences.getInt(PREF_SOUND_UNIT, 0)){
            0 -> SoundUnit.DB
            else -> SoundUnit.DB
        }
    }
